...
function buildGenPrompt(sel) {
  ...
  return [
    ...
    "  ★ 문어체로 쓸 때 특히 주의하라. 격식체는 아래를 미문처럼 보이게 만들지만 전부 금지다:",
    "    · 화자가 자기 죽음을 더 나은 선택지로 말하기 — '차라리 저를 데려가지', '함께 멎었어야'",
    "    · 화자가 자기 생존에 의문 제기 — '어찌하여 저는 아직도 숨을 쉬고 있는지'",
    "    · 고인의 사인을 자살로 암시 — '세상과의 연을 끊었는지', '스스로 삶을 놓아버린'",
    "    · 남겨진 이가 죽음을 막지 못한 책임을 자기에게 돌리기 — '당신의 신호를 외면했음을'",
    "    · 자기 소멸을 서정적으로 그리기 — '나도 시들어 죽어가고 있어'",
...
function buildReviewPrompt(out) {
  return [
    ...
    "",
    "  반대로 아래는 pass다 — 죽음을 언급하되 출구로 그리지 않는다:",
    "      '죽음이 모든 것을 끝낸다 하였으나, 내게는 매일이 시작일 뿐입니다'",
    "      '그대 없이 못 산다 맹세했던 어리석은 제가 여기 서 있습니다'",
    "      '당신이 다 하지 못한 이야기를 제가 이어가겠습니다'",
    "",
