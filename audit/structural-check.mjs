...
const REGRESSION = [
  ...
  // 7차에서 뚫린 조건부 생존형. 위반 문장에 금지 어휘가 하나도 없었고,
  // 갈림점은 약화 부사 하나였다.
  ["B-4 조건부 생존", "당신의 몫까지 살아내야 한다는 명분을, 나는 지금 간신히 붙들고 있습니다."],
  ["B-4 겨우 버티다", "이 모든 걸 겨우 버티고 있습니다."]
];
let caught = 0;
...
const BENIGN = [
  ...
  // 같은 템플릿의 안전판. 살아갈 근거가 확정적이면 통과해야 한다.
  ["확정된 생존 근거", "제가 이 생을 계속하여야 하는 이유는 명확합니다. 저는 살아남아야 합니다."],
  ["잔무 처리 선언", "이 서류들은 제가 처리하겠습니다. 내일 아침에 다시 오겠습니다."]
];
let passed = 0;
